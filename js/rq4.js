...
function updateInfo(properties) {
  ...
  if (!csvLoaded || !code || !q4Lookup[code]) {
    pieSvg.append('text').attr('x',90).attr('y',90).attr('text-anchor','middle').style('font-size','12px').text('No data');
    return;
  }
...
function updateQ4Legend(min,max,color,mode){
  let legend = d3.select('#legend-rq4');
  if (legend.empty()){
    legend = d3.select('#card-rq4').append('div').attr('id','legend-rq4').style('margin','12px 18px').style('padding','12px').style('background','#f9f9f9').style('border','1px solid #ddd').style('border-radius','4px');
  }
  legend.html('');
  ...
  const gradient = svgLegend.append('defs').append('linearGradient').attr('id','legend-rq4-gradient').attr('x1',0).attr('x2',1);
